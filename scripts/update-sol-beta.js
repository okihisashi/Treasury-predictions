...
const EVENT_SLUG        = 'what-price-will-solana-hit-before-2027';
...
async function getSolSpotPrice() {
  const url = `${COINGECKO_API}/simple/price?ids=${SOL_COINGECKO_ID}&vs_currencies=usd`;
  const data = await fetchJSON(url);
  const price = data?.[SOL_COINGECKO_ID]?.usd;
  if (!price) throw new Error('Could not fetch SOL spot from CoinGecko');
  return price;
}
...
async function main() {
  ...
  console.log('============================================');
  console.log('SOL β Updater (Method B)');
  if (dryRun) console.log('🧪 DRY RUN: predictions/sol.json will NOT be written');
  ...
  console.log('\nFetching SOL spot price (CoinGecko)...');
  const currentPrice = await getSolSpotPrice();
  ...
  if (dryRun) {
    ...
    const outPath = 'predictions/sol.json';
